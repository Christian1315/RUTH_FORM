...
class ELECTRICITY_HOUSE_STOP_STATE_HELPER extends BASE_HELPER
{
    ##======== ELECTRICITY HOUSE STOP STATE VALIDATION =======##
    ...
    static function stopStatsOfHouse($request)
    {
        ...
        ####___
        return self::sendResponse($house, 'L\'état en électricité de cette maison a été arrêté avec succès!');
    }
    ...
    function _retrieveHouseStates($request, $houseId)
    {
        ...
        $states = StopHouseElectricityState::with(["Owner", "House", "StatesFactures"])->where(["house" => $houseId])->get();
        return self::sendResponse($states, "Tout les états de la maison " . $house->name . " récupérés avec succès!");
    }

    function _imprimeElectricityHouseState($stateId)
    {
        $state = StopHouseElectricityState::with(["House", "StatesFactures"])->find($stateId);

        if (!$state) {
            return self::sendError("Cet état n'existe pas", 404);
        }
        ###___
        $data["state_html_url"] = env("APP_URL") . "/$stateId/show_electricity_state_html";
        ###__
    ...
    {
        $state = StopHouseElectricityState::with(["Owner", "House", "StatesFactures"])->find($id);
        return self::sendResponse($state, "Etat  récupérés avec succès!");
    }

    function _getAllStates($request)
    {
        $states = StopHouseElectricityState::orderBy("id", "desc")->with(["Owner", "House", "StatesFactures"])->get();
